...
import {
    ...
    minuteInMilisec,
    indexOfFifthWord,
} from '../../../core/constants/sprint-game-const';
...
export class SprintGameController {
    ...
    private endGameTimeout() {
        ...
        this.timerTimeout = setTimeout(() => {
            if (timer && Number(timer.textContent) === 0) {
                const point = document.querySelector('.point-count') as HTMLSpanElement;
                this.score = `Отличная игра! Ваш результат: ${point.textContent}`;
                this.modal.renderModalWindow(this.score);
                this.disableButtons();
        ...
        answersButtons.addEventListener('click', (event) => {
            ...
            if (target.classList.contains('answer-btn')) {
                ...
                if (offset < maxOffSet) {
                    ...
                    this.storage.set('firstIndexOfWord', firstIndexOfWord);
                    this.storage.set('secondIndexOfWord', secondIndexOfWord);
                    this.sprintGameView.clickOnTheAnswerButton(
                        words[firstIndexOfWord].word,
                        words[secondIndexOfWord].wordTranslate
                    );
    ...
    private countPointsWithCorrectAnswer(
        rightAnswerCount: number,
        sumOfCountPoint: number,
        point: number,
        countCircle: number
    ) {
